...
| **업데이트** | 새로운 비즈니스 특성 반영 (신규 유저 유치, 재구매율 향상 집중) |
...
### 비즈니스 현황
- **비즈니스 단계**: 새로운 비즈니스 (밑바닥부터 시작)
- **운영 기간**: 본격 시작한 지 약 2년
- **일본향 서비스**: 2025년 3월부터 시작 (데이터 축적 중)
- **핵심 목표**: 
  - 🎯 **신규 유저 유치** (신규 고객 확보에 집중)
  - 🔄 **재구매율 향상** (1회 구매 고객 → 재구매 고객 전환)

### 비즈니스 특성에 따른 분석 전략

#### 1. 이탈 기준 조정
- **기존**: 2년(730일) 이상 미구매 고객을 이탈 완료로 분류
- **변경**: **6개월(180일)** 이상 미구매 고객을 이탈 완료로 분류
- **근거**: 
  - 새로운 비즈니스이므로 고객 재구매 주기가 짧음
  - 6개월 이상 미구매 시 재활성화 가능성 낮음
  - 신규 유저 유치에 집중하기 위해 이탈 고객은 별도 관리

#### 2. 신규 유저 유치 분석 강화
- **신규 유저 획득 채널 분석**: 어떤 채널에서 유입되는지 분석
- **신규 유저 전환율 분석**: 방문 → 가입 → 첫 구매 전환율
- **신규 유저 품질 분석**: 첫 구매 후 재구매 가능성 예측
- **신규 유저 획득 비용(CAC) 분석**: 채널별 CAC 및 ROI 분석

#### 3. 재구매율 향상 분석 강화
- **1회 구매 고객 분석**: 재구매 전환율이 낮은 고객 특성 분석
- **재구매 전환 시점 분석**: 첫 구매 후 언제 재구매하는지 분석
- **재구매 유도 전략**: 재구매율 향상을 위한 맞춤형 액션 제안
- **재구매 예측 모델**: 1회 구매 고객의 재구매 확률 예측

#### 4. 일본향 서비스 데이터 부족 상황 고려
- **데이터 부족 시나리오 대응**: 데이터가 적을 때도 의미 있는 인사이트 제공
- **트렌드 예측 정확도 조정**: 데이터 부족 시 예측 신뢰도 표시
- **비교 분석**: 일본향 vs 기존 서비스 비교 분석
...
**비즈니스 특성 반영**:
- **이탈 기준**: 6개월(180일) 이상 미구매 고객을 이탈 완료로 분류
  - 새로운 비즈니스 특성상 고객 재구매 주기가 짧음
  - 6개월 이상 미구매 시 재활성화 가능성 낮음
  - 신규 유저 유치에 집중하기 위해 이탈 고객은 별도 관리
...
**이탈 기준 (새로운 비즈니스 특성 반영)**:
```typescript
// 6개월(180일) 이상 미구매 고객은 이미 이탈한 것으로 분류
const CHURNED_THRESHOLD_DAYS = 180  // 기존: 730일 (2년) → 변경: 180일 (6개월)
...
#### 3.1.4 신규 유저 유치 분석 (신규)

**현재 상태**: 신규 유저 획득 관련 분석 부재

**목표 상태**: 신규 유저 획득 전 과정 분석 및 최적화

```typescript
interface NewUserAcquisition {
  // 채널별 분석
  channels: {
...
**현재 상태**: 재구매율 관련 심층 분석 부재

**목표 상태**: 1회 구매 고객 → 재구매 고객 전환율 향상
...
### 4.1 Phase 1: 분석 품질 향상 (3주)

| 주차 | 작업 | 우선순위 | 예상 공수 | 산출물 |
|------|------|---------|----------|--------|
| 1주 | 다차원 교차 분석 구현 | P0 | 3일 | `MultiDimensionalAnalyzer.ts` |
| 1주 | 시계열 분해 분석 구현 | P0 | 2일 | `TimeSeriesDecomposer.ts` |
| 2주 | 고객 이탈 예측 모델 (이탈 기준 조정) | P0 | 2일 | `ChurnPredictor.ts` (180일 기준) |
| 2주 | 신규 유저 유치 분석 | P0 | 3일 | `NewUserAcquisitionAnalyzer.ts` |
| 3주 | 재구매율 향상 분석 | P0 | 3일 | `RepurchaseAnalyzer.ts` |
| 3주 | 작가 건강도 점수 | P1 | 2일 | `ArtistHealthCalculator.ts` |

**Phase 1 완료 기준**:
- [ ] 3차원 이상 교차 분석 가능
- [ ] 시계열 분해 (추세/계절성/잔차) 제공
- [ ] 고객별 이탈 확률 및 위험 요인 제공 (6개월 기준)
- [ ] 신규 유저 획득 채널 분석 및 전환율 최적화
- [ ] 1회 구매 고객 재구매 예측 및 전환율 향상 전략
- [ ] 작가별 4차원 건강도 점수 제공
...
| 순위 | 기능 | 영향도 | 노력 | 의존성 | 최종 우선순위 | 비고 |
|------|------|--------|------|--------|--------------|------|
| 1 | 신규 유저 유치 분석 | 🔴 높음 | 🟠 중간 | 없음 | **P0** | 신규 유저 획득 최적화 |
| 2 | 재구매율 향상 분석 | 🔴 높음 | 🟠 중간 | 없음 | **P0** | 1회 구매 → 재구매 전환 |
| 3 | 이탈 예측 (180일 기준) | 🔴 높음 | 🟢 낮음 | RFM 데이터 | **P0** | 기준 조정 완료 |
| 4 | 인사이트 → 액션 연결 | 🔴 높음 | 🟢 낮음 | 없음 | **P0** | 완료 |
| 5 | 다운로드 기능 | 🟠 중간 | 🟢 낮음 | 없음 | **P0** | 완료 |
| 6 | 통계적 유의성 검증 | 🟠 중간 | 🟠 중간 | 없음 | **P1** | 데이터 부족 시나리오 고려 |
| 7 | 작가 건강도 점수 | 🟠 중간 | 🟠 중간 | 작가 데이터 | **P1** | 완료 |
| 8 | 다차원 교차 분석 | 🟠 중간 | 🔴 높음 | 데이터 구조 | **P1** | 일본향 서비스 데이터 고려 |
| 9 | 시계열 분해 분석 | 🟠 중간 | 🟠 중간 | 없음 | **P1** | 데이터 부족 시 신뢰도 표시 |
| 10 | 인과관계 추론 | 🟠 중간 | 🔴 높음 | 통계 모듈 | **P2** | - |
| 11 | What-if 시뮬레이션 | 🟠 중간 | 🔴 높음 | 예측 모델 | **P2** | - |
